...
\draw[dashed] (\uangle:1) |- (0,0);
\draw[decorate,decoration={brace,amplitude=4pt,mirror,raise=2pt}] (0,0) -- node[below,yshift=-2mm] {$r\cos\alpha$} ({cos(\uangle)},0);
\draw[decorate,decoration={brace,amplitude=4pt,raise=2pt}] (0,0) -- node[left,xshift=-2mm] {$r\sin\alpha$} (0,{sin(\uangle)});
...
De lengte van de wijzer is $r$ en de hoek van de wijzer ten opzichte van de reële as is $\alpha$. We kunnen het punt $\underline{z}$ nu ook beschrijven met behulp van de lengte en de hoek:
%
\begin{equation}
\underline{z} = a + \imunit b = r\cos\alpha + \imunit r\sin\alpha = r(\cos\alpha + \imunit\sin\alpha)
\end{equation}
%
