...
schakelschema's zijn te zien in figuur~\ref{fig:symbolenbronnenenweerstand}. Een
...
levert niet afhankelijk is van een andere spanning of stroom in een netwerk. We zullen
verderop kennis maken met afhankelijke bronnen.
...
\section{De stroomwet van Kirchhof}
...
van 10 V en een stroom van de stroombron van 0,5 A. Verder zijn de weerstandswaarden
$R_1 = 6\ \Omega$, $R_2 = 8\ \Omega$ en $R_3 = 3\ \Omega$. In
figuur~\ref{fig:gelnetwerkvoorsuperpositiebeginsel2} is het schema te zien.

\begin{figure}[!ht]
\centering
\begin{tikzpicture}[bookcircuit]
\draw (0,0) to [V<=10 V] ++(0,2) to [R=$6\ \Omega$, -*] ++(2,0) node[above] {$U_x$}
            to [R=$3\ \Omega$, i>^=$I_x$,-*] ++(0,-2) to [open] ++(0,2) to [R=$8\ \Omega$] ++(2,0)
            to [I_>=, l=$\frac{1}{2}$ A, invert] ++(0,-2) to [short, -.] (0,0);
\end{tikzpicture}
\caption{Netwerk voor berekening met superpositie.}
\label{fig:gelnetwerkvoorsuperpositiebeginsel2}
\end{figure}

We splitsen dit schema in twee deelschema's. Deze zijn te zien in
...
\centering
\begin{tikzpicture}[bookcircuit]
\draw (0,0) to [V<=10 V] ++(0,2) to [R=$6\ \Omega$, -*] ++(2,0) node[above] {$U_{xa}$}
            to [R=$3\ \Omega$, i>^=$I_{xa}$,-*] ++(0,-2) to [open] ++(0,2) to [R=$8\ \Omega$] ++(2,0)
            to [open] ++(0,-2) to [short, -.] (0,0);
\end{tikzpicture}
\caption{Berekening vanuit de spanningsbron.}
...
\centering
\begin{tikzpicture}[bookcircuit]
\draw (0,0) to [short] ++(0,2) to [R=$6\ \Omega$, -*] ++(2,0) node[above] {$U_{xb}$}
            to [R=$3\ \Omega$, i>^=$I_{xb}$,-*] ++(0,-2) to [open] ++(0,2) to [R=$8\ \Omega$] ++(2,0)
            to [I_>=, l=$\frac{1}{2}$ A, invert] ++(0,-2) to [short, -.] (0,0);
\end{tikzpicture}
\caption{Berekening vanuit de stroombron.}
...
\begin{equation}
\begin{split}
U_{xa} &= \dfrac{3}{3+6}\cdot 10 = 3\frac{1}{3}\text{ V} \\
I_{xa} &= \dfrac{10}{3+6} = 1\frac{1}{9}\text{ A}
\end{split}
\end{equation}
%
...
\begin{equation}
\begin{split}
U_{xb} &= (6//3)\cdot\dfrac{1}{2}  = 1\text{ V} \\
I_{xb} &= \dfrac{6}{3+6}\cdot\dfrac{1}{2} = \frac{1}{3}\text{ A}
\end{split}
\end{equation}
%
...
\begin{equation}
\begin{split}
U_{x} &= U_{ax} + U_{bx} = 3\frac{1}{3} + 1 = 4\frac{1}{3}\text{ V} \\
I_{x} &= I_{ax} + I_{bx} = 1\frac{1}{9} + \frac{1}{3} = 1\frac{4}{9}\text{ A}
\end{split}
\end{equation}
%
...
%
\begin{equation}
P_{R_3} = U_x\cdot I_x = 4\frac{1}{3} \cdot 1\frac{4}{9} = 6\dfrac{7}{27}\text{ W}
\end{equation}
